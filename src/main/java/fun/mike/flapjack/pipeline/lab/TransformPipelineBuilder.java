...
public class TransformPipelineBuilder {
    ...
    public TransformPipelineBuilder(FlatInputFile flatInputFile, List<Operation> steps) {
        this.flatInputFile = flatInputFile;
        this.steps = steps;
    }

    // Factory methods
    public static TransformPipelineBuilder mapFirst(String id, String description, FlatInputFile flatInputFile, Function<Record, Record> mapper) {
        MapOperation operation = new MapOperation(id, description, mapper);
        List<Operation> steps = new LinkedList<>();
        steps.add(operation);
        return new TransformPipelineBuilder(flatInputFile, steps);
    }

    public static TransformPipelineBuilder filterFirst(String id, String description, FlatInputFile flatInputFile, Predicate<Record> predicate) {
